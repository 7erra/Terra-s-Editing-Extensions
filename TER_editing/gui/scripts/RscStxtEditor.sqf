...
// add lb formats
_formatArray = [
	["Line break",format ["<br/>%1",toString [10]]],
...
[] spawn {
	...
	while {!isNull STXTEDITOR_DISPLAY} do {
		...
		_curText = [_curText,"  ",format ["<br/>%1",toString [10]]] call _fnReplaceString;
		ED_INPUT ctrlSetText _curText;
...
ED_INPUT ctrlAddEventHandler ["KeyDown",{
	params ["_control", "_key", "_shift", "_ctrl", "_alt"];
	_curText = ctrlText ED_INPUT;
	if (_ctrl && _key == 0x16) then {
		_curText = _curText +"<t underline='true'>Underline</t>";
		ED_INPUT ctrlSetText _curText;
	};
}];