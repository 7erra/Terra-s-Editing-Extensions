...
switch _mode do {
	case "loadColor":{
		_params params ["_grpColorPicker", ["_rgba",[1,1,1,1]]];
		{
	...
	case "onLoad":{
		_params params ["_grpColorPicker"];
		_loadColor = uiNamespace getVariable ["TER_3den_ctrlColorPicker_loadColor",[1,1,1,1]];
	...
	case "updateColor": {
		_params params ["_slider", "_newValue"];
		...
		uiNamespace setVariable ["TER_3den_ctrlColorPicker_loadColor", _colorArray];
	};
	case "keyEH":{
		_params params ["_grpColorPicker","_key"];
